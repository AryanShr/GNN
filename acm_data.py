...
def create_acm_dgl_graph(dowload_path='/tmp/ACM.mat',
    ...
    g = dgl.heterograph({
        ...
        ('game', 'based-on', 'topic'): (th.tensor([3, 4]), th.tensor([4, 5]))
    })
    # Node and Edge Features
    g.nodes['user'].data['U'] = th.ones(4, 1)
    g.nodes['game'].data['G'] = th.ones(5,1)
    g.nodes['topic'].data['T'] = th.ones(6,1)

    g.edges['follows'].data['UvsT'] = th.ones(2,1)
    g.edges['plays'].data['UvsG'] = th.ones(2,1)
    g.edges['based-on'].data['GvsT'] = th.ones(2,1)
    ...
    text_feat = {
        'user': ['intelligent', 'dumb', 'average', 'genius'],
        'topic': ['Action','Adventure', 'Fantasy', 'Magic', 'Sci-Fi','Mecha'],
        'game': ['Legend of Zelda', 'Final Fantasy', 'Super Mario', 'Pokemon', 'Fire Emblem']
    }
    ...
    # pvc = ['intelligent', 'dumb', 'average', 'genius'].tocsr()
    # p_selected = pvc.tocoo()
    # # generate labels
    # labels = pvc.indices
    labels = th.tensor([0,1,2,3]).long()
    g.nodes['user'].data['label'] = labels
    ...
    for n_type in g.ntypes:
        emb = nn.Parameter(th.Tensor(g.number_of_nodes(n_type), 256), requires_grad = False)
        nn.init.xavier_uniform_(emb)
        g.nodes[n_type].data['feat'] = emb
    ...
    if output_path is not None:
        ...
        output_graph_file_path = os.path.join(output_path, dataset_name + '.dgl')
        print(f'Saving ACM data to {output_graph_file_path} ......')
        save_graphs(output_graph_file_path, [g], None)
        print(f'{output_graph_file_path} saved.')
        # Save raw node text
        output_text_file_path = os.path.join(output_path, dataset_name + '_text.pkl')
        print(f'Saving ACM node text to {output_text_file_path} ......')
